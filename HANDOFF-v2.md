...
| **Hero** | Captura real do beFranq | A tela do MacBook é ilustração de produto com dados de exemplo. **A legenda visível “Ilustração da plataforma · dados de exemplo” foi removida a pedido** — a ressalva sobrevive só no `aria-label`, que abre com “Ilustração de um MacBook…”. Um visitante que vê a tela não é mais avisado de que 18 clientes e 7 propostas são exemplo; se quiser a ressalva de volta sem poluir o hero, o lugar natural é o texto legal do rodapé. Para trocar por captura real: substituir o conteúdo de `.fq-mac__ui` por `<img class="fq-mac__shot" src="..." alt="">`. **Decisão sua:** uma captura do ambiente logado levaria dados de PBs e clientes reais para uma página pública — o caminho seguro é uma conta de demonstração com dados fictícios |
